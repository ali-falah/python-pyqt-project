...
class main(QtWidgets.QMainWindow,Form_class):
    ...
    def __init__(self):
        ...
        self.setFixedSize(731,761)
        self.handel_buttons()
        self.Qtime = QTimer(self)
        self.Qtime.timeout.connect(self.timer)
    ...
    def handel_buttons(self):
        ...
        self.toolButton.clicked.connect(self.submituser)
        ...
        self.pushButton_19.clicked.connect(self.open_1)
        self.pushButton_18.clicked.connect(self.open_2)
        self.pushButton_20.clicked.connect(self.save_excel_1)
        self.pushButton_21.clicked.connect(self.save_excel_2)
        self.lineEdit.returnPressed.connect(self.login)
        self.lineEdit_2.returnPressed.connect(self.login)
        self.LINEEDIT_up_del.returnPressed.connect(self.search_costumer)
        self.lineEdit_5.returnPressed.connect(self.search_Employee)
        self.lineEdit_7.returnPressed.connect(self.search_Employee)

    def save_excel_1(self):
        self.path = QFileDialog.getSaveFileName(self,'CHOOSE FOLDER AND FILE NAME','C:\\','.xlsx')
        copyfile('costumer_info.xlsx',self.path)
        self.statusbar.showMessage('FILE SAVED SUCCESSFULLY :)')
        self.sleeper()

    def save_excel_2(self):
        self.path = QFileDialog.getSaveFileName(self,'CHOOSE FOLDER AND FILE NAME','C:\\','.xlsx')
        copyfile('employee_info.xlsx',self.path)
        self.statusbar.showMessage('FILE SAVED SUCCESSFULLY :)')
        self.sleeper()
    ...
    def open_1(self):
        webbrowser.open("costumer_info.xlsx")

    def open_2(self):
        webbrowser.open("employee_info.xlsx")
    ...
    def logout(self):
        ...
        if ret == QtGui.QMessageBox.Yes:
            self.stackedWidget.setCurrentIndex(0)
            self.stackedWidget.resize(731,761)
            self.setFixedSize(731,761)
            self.move(100,100)
            self.statusbar.showMessage('DEVELOPED BY ALI FALAH ')
            return
...
class About_Window(QtWidgets.QDialog,Window_class):
